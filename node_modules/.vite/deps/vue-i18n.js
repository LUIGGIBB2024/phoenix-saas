...
function compile(message, context) {
  if (!__INTLIFY_DROP_MESSAGE_COMPILER__ && isString2(message)) {
...
function createMessageContext(options = {}) {
  ...
  function message(key, useLinked) {
    const msg = isFunction2(options.messages) ? options.messages(key, !!useLinked) : isObject2(options.messages) ? options.messages[key] : false;
...
function translate(context, ...args) {
  ...
  const defaultMsgOrKey = isString2(options.default) || isBoolean2(options.default) ? !isBoolean2(options.default) ? options.default : !messageCompiler ? () => key : key : fallbackFormat ? !messageCompiler ? () => key : key : null;
  const enableDefaultMsg = fallbackFormat || defaultMsgOrKey != null && (isString2(defaultMsgOrKey) || isFunction2(defaultMsgOrKey));
  const locale = getLocale(context, options);
