...
class ScrapingController extends Controller
{
    ...
    public function login(Request $loginrequest)
    {
        ...
        try 
        {
                // 🧩 1️⃣ Configurar cookies y cliente con certificado
                $jar = new CookieJar();

                $clientCert = new Client([
                    'base_uri' => rtrim($endpoint_main, '/'),
                    'cert'     => $certPem,
                    'ssl_key'  => $keyPem,
                    'verify'   => false, // ⚠️ sólo para pruebas locales
                    'cookies'  => $jar,
                    'headers'  => [
                ...
                // 🧩 2️⃣ Cargar página inicial con certificado
                $response = $clientCert->get('/User/CertificateLogin', ['cookies' => $jar]);
                $html = (string) $response->getBody();

                $crawler = new Crawler($html);
                $csrf = $crawler->filter('input[name="__RequestVerificationToken"]')->count() 
                    ? $crawler->filter('input[name="__RequestVerificationToken"]')->attr('value')
                    : null;

                // Acción del formulario (si no existe, usar fallback)
                $formAction = $crawler->filter('form')->count()
                    ? $crawler->filter('form')->attr('action')
                    : '/User/CertificateAuthentication';
                ...
                // 🧩 4️⃣ Detectar si ASP.NET devolvió otro formulario (redirección interna)
                $crawler = new Crawler($html);

                if ($crawler->filter('form')->count() > 0) {
                    $nextAction = $crawler->filter('form')->attr('action');
                    $hiddenInputs = $crawler->filter('form input')->each(function ($node) {
                    ...
                    foreach ($hiddenInputs as $input) {
                        if (isset($input)) {
                            $formParams[key($input)] = current($input);
                        }
                    }

                    // 🧩 5️⃣ Reenviar el formulario oculto manualmente (simular auto-submit)
                    $response = $clientCert->post($nextAction, [
                ...
                // 🧩 6️⃣ Verificar si llegamos al dashboard u otra página protegida
                $status = str_contains($html, 'Bienvenido') 
                    ? '✅ Acceso exitoso'
                    : '⚠️ Página intermedia o error';

                return response()->json([
                    'status' => $status,
                    'csrf' => $csrf,
                    'first_action' => $formAction,
                    'http_code' => $response->getStatusCode(),
                    'final_url' => $response->getHeader('X-Guzzle-Redirect-History')[0] ?? $formAction,
                    'html_snippet' => substr($html, 0, 400),
                ]);
