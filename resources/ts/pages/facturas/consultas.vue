...
const showDetailDialog = async (item: any) => {
  ...
  try {
    ...
    const { data } = await axios.post(`/api/facturas-detalle/${item.id}`, {
      ...
      sales_invoices_id: selectedInvoice.value.id,
    },
    {
    ...
    console.log('Respuesta Det API:', data)
...
const generarConsulta = async () => {
  ...
  try {
    // onsole.log("Generando Consulta con Fechas:", datafechas.value.desdefecha, datafechas.value.hastafecha, "Page:", page.value, "Items/Page:", itemsPerPage.value)
    const { data } = await axios.post('/api/facturas-consultas', {
      url_token: localStorage.getItem('auth_token'),
      company_id: localStorage.getItem('company_id'),
      fechadesde: datafechas.value.desdefecha,
      fechahasta: datafechas.value.hastafecha,
...
const totalRentabilidad = computed(() => invoiceData.value.TotalRentabilidad ?? 0)
...
const exportarPDF = () => {
  ...
  autoTable(doc, {
    ...
    foot: [[
      ...
      formatCurrency(totalRentabilidad.value),
    ]],
...
const headers = [
  ...
  { title: 'Costo de Venta', key: 'cost_of_sale', sortable: true },
  { title: 'Total Documento', key: 'total_sale', sortable: true },
  { title: 'Márgen', key: 'rentabilidad', sortable: true },
  { title: '(%%)', key: 'margen', sortable: true },
  { title: 'Acción', key: 'actions', sortable: true },
]
...
function calcularMargen(item: any): string {
  if (!item.margen || isNaN(item.margen))
    return '0.00'

  return item.margen.toFixed(2)
}
</script>
...
<template>
      ...
      <div class="table-responsive">
        ...
        >
          ...
          <template #header.customer>
            <div class="th-center text-column">
          ...
          <template #item.cost_of_sale="{ item }">
            <div class="td-right text-column">
              {{ formatCurrency(item.cost_of_sale) }}
            </div>
          </template>
          <template #item.rentabilidad="{ item }">
            <div class="td-right text-column">
              {{ formatCurrency(item.rentabilidad) }}
            </div>
          </template>
          ...
          <template #header.cost_of_sale>
            <div class="th-center">
              Costo de Venta
            </div>
          </template>
          <template #item.actions="{ item }">
            <IconBtn @click="showDetailDialog(item)">
              <VIcon
                icon="tabler-list"
                color="primary"
                size="15"
  ...
  >
    <VCard>
      <VCardTitle class="modal-title text-h5">
        Detalle de Factura
      </VCardTitle>
      <VCardText class="pa-4">
        <VTable
          v-if="invoiceDetData.data.length"
        ...
        >
          ...
          <tbody>
            <tr
              v-for="(item, index) in invoiceDetData.data"
              :key="item.id"
              py-0
            >
              <td width="10%">
              ...
              <td>
                <div class="text-subtitle-2 text-right">
                  {{ formatCurrency(item.discount1) }}
                </div>
              </td>
              <td>
                <div class="text-subtitle-2 text-right">
                  {{ formatCurrency(item.vat_percentage, 0) }}
                </div>
              </td>
              <td>
                <div class="text-subtitle-2 text-right">
