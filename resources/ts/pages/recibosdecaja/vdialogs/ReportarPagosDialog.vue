...
import type { CxCPayment, SalesInvoice } from '..'
...
const emit = defineEmits(['update:modelValue', 'pago-reportado', 'close', 'closeReg', 'cerrar-todo', 'recibo-guardado'])
...
interface Props {
  ...
  tipoDeRecibo: string
  isFocused: boolean
  tercero?: CxCPayment // 👈 agregar aquí también
  itemsdetallefact: SalesInvoice[] // 👈 Usar el tipo real en vez de []
  totales: []
  valorrecibo: number
...
// 3. Variables locales
const itemDetalleFact = ref<SalesInvoice[]>([])
...
function AgregarPagosDirectos(factura: SalesInvoice) {
  const cptopago = '999'
  const valorpago = ref(Number.parseFloat(factura.valor_factura)) // 👈 Valor directo, sin ref()

  console.log('Total Capturado - 200:', totalCapturado.value, ' Valor Recibo :', props.valorrecibo)

  if (totalCapturado.value < props.valorrecibo) {
    const saldoactual = props.valorrecibo - (totalCapturado.value)
    ...
    if (!tienePagosEstaFactura && props.paymentcpt?.length) {
      ...
      console.log('Agregue una registro a pagosTemporal para la factura:', factura.id, ' Nuevos Pagos:', nuevosPagos)
    ...
    console.log('Soy itemDetalleFact antes de actualizar:', factura.id, ' ItemDetalle:', itemDetalleFact.value, 'Registro Actualizado:', registroActualizado)
    ...
    const registroExistente = pagosTemporal.value.find(
      pago => Number(pago.factura_id) === Number(factura.id) && String(pago.concepto) === String(cptopago),
    )

    console.log('Soy Registro Existente:', pagosTemporal.value, 'Factura:', factura.id, ' Registro Existente:', registroExistente, ' CptoPago:', cptopago, ' Valor Pago:', valorpago.value)

    if (registroExistente) {
      // Si ya existe en pagosTemporal, le asignamos el valor del pago
      console.log('Registro existente encontrado para la factura:', factura.id, 'Actualizando valor a:', valorpago.value)
    ...
    console.log('Total Capturado - 220:', totalCapturado.value, ' Saldo Actual:', saldoactual, ' valora Factura:', factura.valor_factura, 'Aplicación:', totalCapturado.value, 'Conceptos:', props.paymentcpt)
  }
}
...
const handleSaveRecibo = async (recibo: any) => {
  try {
    const response = await axios.post('/api/customerpayment', {
      ...recibo, // Ahora sí, este 'recibo' es un objeto JavaScript 100% normal
      tipo: 'FACTURAS',
    ...
    emit('recibo-guardado', {
      esEdicion: !!recibo.id,
      registro: response.data.payments,
    })

    showSnackbar.value = true
    snackbarMessage.value = '<< Recibo Generado Exitosamente >>'
  ...
  >
    <VCard>
      ...
      <VCardActions>
        <VSpacer />
        <div class="cell-wrap align-end">
          <span style="font-size: 0.78rem;">Valor del Recibo: </span>
          <VChip
            size="x-small"
            variant="flat"
            class="rounded-pill"
        ...
        <VBtn
          ...
          color="success"
          variant="flat"
          :disabled="props.valorrecibo !== totalCapturado"
          @click="handleSaveRecibo(props.tercero)"
        >
          Guardar
        </VBtn>
      </VCardActions>
