...
const totalesFactura = computed(() => {
  ...
  recordComprasT.value.total = totalGeneral
...
function agregarProducto(): void {
  ...
  if (!editMode.value) {
    ...
    // --- MODO EDITAR ---
    const updatedRecord = newRecordCargue.value

    console.log('Soy Updaterecod: ', updatedRecord)

    // 1. Buscar el índice del producto en el detalle actual usando su ID
    const index = itemDetalle.value.findIndex(item => item.idregistro === updatedRecord.idregistro)

    if (index !== -1) {
      // 2. Actualizamos el registro manteniendo la estructura del detalle
      itemDetalle.value[index] = {
        ...itemDetalle.value[index], // Mantiene campos fijos si los hay (ej: store, subtotal anterior)
        ...
        // Aquí puedes recalcular totales si los necesitas en caliente, por ejemplo:
        // total: updatedRecord.quantity * updatedRecord.cost
      }

      // Nota Importante sobre Reactividad:
      // Si tu itemDetalle es un ref() normal, Vue detectará el cambio de inmediato.
      // Si por alguna razón itemDetalle fuese un computed de lectura/escritura o necesitas forzar la actualización:
      itemDetalle.value = [...itemDetalle.value]
    }
  }
  editMode.value = false
  productoSeleccionado.value = null // <--- ¡MUY IMPORTANTE!
  productoInfo.value = null // <--- Limpia también tu estado auxiliar
...
function Limpiar_RegCargue(): void {
  ...
  newRecordCargue.value.idregistro = 0
  ...
  // 2. Limpieza segura de productoSeleccionado (EL CULPABLE)
  if (productoSeleccionado.value) {
    productoSeleccionado.value.code = ''
    productoSeleccionado.value.name = ''
  }
  else {
    // Si es null, lo inicializamos como un objeto vacío para que no falle después
    productoSeleccionado.value = { code: '', name: '' }
  }
}

function Procesar_Descuentos(): void {
  // Recorremos el array itemDetalle
  itemDetalle.value.forEach(item => {
    // 1. Verificamos si el descuento es mayor a cero
    if (item.discount > 0) {
      // Calculamos el valor del descuento
      // Nota: Si 'discount' es un porcentaje (ej: 10 para 10%), usa: (item.cost * item.discount) / 100
      // Si 'discount' es un valor fijo, usa: item.discount
      const valorDescuento = item.discount * item.cost / 100

      // 2. Aplicamos el descuento al campo 'cost'
      item.cost = item.cost - valorDescuento

      // 3. Reseteamos el campo 'discount' a cero
      item.discount = 0
    }
  })

  console.log('Soy Item con Descuentos Cambiados:', itemDetalle.value)
}

function Procesar_Iva(): void {
  // Recorremos el array itemDetalle
  itemDetalle.value.forEach(item => {
    // 1. Verificamos si el descuento es mayor a cero
    if (item.vat > 0) {
      // Calculamos el valor del descuento
      // Nota: Si 'discount' es un porcentaje (ej: 10 para 10%), usa: (item.cost * item.discount) / 100
      const _costo = Number.parseFloat(item.cost)
      const _iva = Number.parseFloat(item.vat)

      // Si 'discount' es un valor fijo, usa: item.discount
      const valorIva = (_costo * (_iva / 100))

      // 2. Aplicamos el descuento al campo 'cost'
      item.cost = Number.parseFloat(item.cost) + valorIva
...
const saveRecordCompras = async () => {
  ...
  try {
    ...
    const registroActualizado = {
      ...compraRecibida,
      concept_name: conceptNameRecibido,
    }
    ...
    showComprasDialog.value = false
    showSaveChargesDialog.value = false
  }
  catch (error: any) {
    // 1. Extraer el mensaje detallado del servidor
    const serverMessage = error.response?.data?.message

    // 2. Extraer errores de validación (si existen)
    // Laravel devuelve los errores de validación en un objeto 'errors'
    const validationErrors = error.response?.data?.errors
    let detailedError = ''

    if (validationErrors) {
      // Convertimos el objeto de errores en un solo string legible
      detailedError = Object.values(validationErrors).flat().join(' | ')
    }

    // 3. Decidir qué mostrar en el snackbar
    // Prioridad: Errores de validación > Mensaje del servidor > Mensaje genérico
    snackbarMessage.value = detailedError || serverMessage || 'Error inesperado al guardar la compra'

    // Log para el desarrollador (más completo)
...
const saveRecord = async () => {
  ...
  try {
    ...
    // ✅ 1. Extraemos la compra y el nombre del concepto de la respuesta
    const compraRecibida = data.purchase
    const conceptNameRecibido = data.concept_name ?? 'COMPRAS'
...
const openEditCargues = (item: any) => {
  showCarguesDialog.value = true
  newRecordCargue.value.store = item.store
  newRecordCargue.value.quantity = item.quantity
  newRecordCargue.value.discount = item.discount
  newRecordCargue.value.vat = item.vat
  newRecordCargue.value.cost = item.cost
  newRecordCargue.value.id = item.id
  newRecordCargue.value.idregistro = item.idregistro
  newRecordCargue.value.code = item.code
  newRecordCargue.value.name = item.name
  editMode.value = true

  // 2. SOLUCIÓN: Armar el objeto para el v-model del VAutocomplete
  // Debe tener la estructura exacta de un producto de tu array 'products'
  productoSeleccionado.value = {
    id: item.product_id || item.id, // Asegúrate de pasar el ID del PRODUCTO, no el ID del registro de la tabla si son distintos
    name: item.name,
    cost: item.cost,
  }
}
                  ...
                  >
                    ...
                    >
                      ...
                      <template #item.quantity="{ item }">
                        <div class="cell-wrap columna_name2">
                          {{ formatCurrency(item.quantity, 2) }}
                        </div>
                      </template>

                      <template #item.discount="{ item }">
                        <div class="cell-wrap columna_name2">
                          {{ formatCurrency(item.discount, 2) }}
                        </div>
                      </template>
              ...
              >
                <template #activator="{ props }">
                  <VBtn
                    ...
                    icon
                    class="mx-3"
                    style="background-color: #fc4a3d !important;"
              ...
              >
                <template #activator="{ props }">
                  <VBtn
                    ...
                    icon
                    class="mx-3"
                    style="background-color: #fc039c !important;"
                    @click="Procesar_Descuentos"
                  >
                    <VIcon
              ...
              >
                <template #activator="{ props }">
                  <VBtn
                    ...
                    height="37"
                    icon
                    style="background-color: #1cbb0e !important;"
                    @click="Procesar_Iva"
                  >
                    <VIcon
    ...
    <VCard>
      ...
      <VCardText class="pt-0 pb-0">
        ...
        >
          <VForm>
            ...
            >
              ...
              >
                <AppTextField
                  v-model="costoFormateado"
                  label="$ Costo Actual"
                  required
                  class="mb-2 text_size aligned-field text-error"
                  :rules="[rules.required]"
                  placeholder="Descuento"
                  density="comfortable"
                  variant="outlined"
        ...
        <VRow
          ...
          class="justify-end"
        >
          <VCardActions class="mt-8 px-2 mb-3 mx-4 mt-4">
            <VBtn
              color="error"
              variant="flat"
              class="text-white"
