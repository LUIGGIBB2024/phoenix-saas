...
const totalDocuments = computed(() =>
  documentosFiltrados.value.reduce((acc, item) => acc + (Number(item.value_cxp) || 0), 0),
)
...
const generarConsulta = async () => {
  ...
  try {
    ...
    const { data } = await axios.post('/api/consult-payments-cxp', {
...
const headers = [
  ...
  { title: 'Descripción Documento', key: 'document_name', sortable: true },
